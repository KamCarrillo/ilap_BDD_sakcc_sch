-- @Descripción: API BLOB FACTURA en schbdd_s1 (local = servicio_laptop_f1)
...
begin
  ...
  if sql%rowcount <> 1 then
    raise_application_error(-20020,'No se encontró servicio_laptop_f1 para actualizar FACTURA');
  end if;
end;
/
show errors
...
begin
  select factura into v from servicio_laptop_f1
...
begin
  select factura into v from servicio_laptop_f2@schbdd_s2.fi.unam
   where num_servicio=p_num_servicio and laptop_id=p_laptop_id;
  return v;
exception when no_data_found then return null;
end;
